...
1. **Stock reservation works — into the warehouse the integration settings name.** Resolved 2026-09-08: the paid sandbox order came back `is_reserved: true`, `reserved_pids`, `mid: "51630"`, and moved the work `instock 1 → 0` with `quantity` untouched. Task 2's earlier no-reservation result was the same rule seen from the other side: that token reserved into 34998, where the mock product had no stock row. **Consequence for the cutover:** whichever token production uses must have its reservation warehouse set to 34998, or paid works will not be reserved and a sold painting stays purchasable. Check it on the integration settings page before go-live.
