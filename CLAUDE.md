...
- **Images are CRM-hosted** (`https://crm.h-profit.com/bimages/get/...`). Allowlist that host in `next.config.ts` `images.remotePatterns` before using `next/image` on products.
